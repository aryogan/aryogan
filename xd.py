...
def banner():
	...
	sol().print(cik2)
	ban='''
•   AUTHOR : AOREC-XD WHATSAPP : 085696526470   •
...
•                                               •
•   GITHUB : HTTPS://GITHUB.COM/ARYO-GG/4MBF   •'''
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='ARYO-GG CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='ARYO-GG OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]ARYO-GG OK[/bold green]'))
					ok+=1
					break
